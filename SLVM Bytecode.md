...
| `getVarAddress`             | [var]                  | sets the A register to the address of [var]
| `setVarAddress`             | [var]                  | sets the address of [var] to the value of the A register
| `copyVar`                   | [source], [dest]       | copy's [source] to [dest]
| `incA`                      |                        | increments the A register by 1
| `decA`                      |                        | decrements the A register by 1
| `arrayBoundsCheck`          | [array]                | checks if the A register is in range of the size of [array] if not, prints and error then stops the program
| `getValueAtPointerOfA`      |                        | sets the A register to the value at memory address of the value of the A register
| `metadataLine`              | line                   | remembers the source code line, used for debuging
| `metadataFilename`          | fileName               | remembers the source code filename, used for debugging
